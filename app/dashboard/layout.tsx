...
import { PieChart, Inbox, Users, Search, Bell, MapPin } from 'lucide-react';
    ...
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden', flexDirection: 'column', background: 'var(--bg-primary)' }}>
      ...
      <header style={{ height: '64px', background: '#ffffff', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 24px', borderBottom: '1px solid var(--glass-border)', flexShrink: 0, zIndex: 10 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          <h2 style={{ fontSize: '1.2rem', fontWeight: 800, color: 'var(--primary-color)', margin: 0 }}>الميدان</h2>
        ...
        <nav style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <Link href="/dashboard" {...getLinkStyle('/dashboard')}>
            <Inbox size={16} /> الكانبان
            {pathname === '/dashboard' && <span style={{ background: 'var(--danger-color)', color: '#fff', fontSize: '0.6rem', padding: '2px 6px', borderRadius: '10px', marginRight: '4px' }}>12</span>}
          </Link>
          <Link href="/dashboard/analytics" {...getLinkStyle('/dashboard/analytics')}>
            <PieChart size={16} /> التحليلات
          </Link>
          <Link href="/dashboard/citizens" {...getLinkStyle('/dashboard/citizens')}>
            <Users size={16} /> المواطنون
          </Link>
        </nav>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', paddingLeft: '16px', borderLeft: '1px solid var(--glass-border)' }}>
            <div style={{ textAlign: 'left' }}>
               ...
               <p style={{ margin: 0, fontSize: '0.7rem', color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: '4px' }}><MapPin size={10}/> إقليم الوسط</p>
            </div>
